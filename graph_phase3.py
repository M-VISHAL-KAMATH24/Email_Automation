...
class EmailState(TypedDict):
    ...
    snippets: Optional[List[dict]]  # New: Retrieved chunks with sources
    retrieval_weak: Optional[bool]  # New: Flag for weak retrieval
    draft: Optional[str]  # New: Generated draft response
...
async def retrieve_snippets(state: dict) -> dict:
    ...
    if retrieved_docs and len(retrieved_docs) < 3:  # Adjust threshold as needed
...
# New conditional for post-classify: If support/billing and high confidence, go to retrieve
...
async def test_graph():
    ...
    for email in emails:
        ...
        print(f"Thread {result['thread_id']}: Label={result['label']} (Conf={result['confidence']}), Summary={result['summary']}, Draft={result.get('draft', 'N/A')}")
